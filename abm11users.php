...
$sql_abm11 = "SELECT r.lrn, r.role, CASE WHEN u.lrn IS NULL THEN 'Pending' ELSE 'Registered' END AS registration_status
            FROM registrar r
            LEFT JOIN users u ON r.lrn = u.lrn
            WHERE r.role IN ('ABM', 'standby-abm11')";
$result_abm11 = mysqli_query($conn, $sql_abm11);
?>
...
<body>
    <div class="container mt-4">
        ...
        <table class='table table-striped mt-4'>
            ...
            <tbody>
                <?php if(mysqli_num_rows($result_abm11) > 0) : ?>
                    <?php while ($row_abm11 = mysqli_fetch_assoc($result_abm11)) : ?>
                        <tr>
                            <td><?php echo $row_abm11['lrn']; ?></td>
                            <td><?php echo $row_abm11['role']; ?></td>
                            <td><?php echo $row_abm11['registration_status']; ?></td>
                            <td>
                                <?php if ($row_abm11['role'] !== 'ABM') : ?>
                                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                        <input type='hidden' name='lrn' value='<?php echo $row_abm11['lrn']; ?>'>
