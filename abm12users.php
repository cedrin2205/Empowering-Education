...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["submit_add_registrar"])) {
        ...
        $lrn = sanitizeInput($_POST['lrn']);
        $new_role = "ABM12"; // Set role to ABM12
...
$sql_abm12 = "SELECT r.lrn, r.role, CASE WHEN u.lrn IS NULL THEN 'Pending' ELSE 'Registered' END AS registration_status
            FROM registrar r
            LEFT JOIN users u ON r.lrn = u.lrn
            WHERE r.role IN ('ABM12', 'standby-abm12')";
$result_abm12 = mysqli_query($conn, $sql_abm12);
?>
...
<body>
    <div class="container mt-4">
        ...
        <table class='table table-striped mt-4'>
            ...
            <tbody>
                <?php if(mysqli_num_rows($result_abm12) > 0) : ?>
                    <?php while ($row_abm12 = mysqli_fetch_assoc($result_abm12)) : ?>
                        <tr>
                            <td><?php echo $row_abm12['lrn']; ?></td>
                            <td><?php echo $row_abm12['role']; ?></td>
                            <td><?php echo $row_abm12['registration_status']; ?></td>
                            <td>
                                <?php if ($row_abm12['role'] !== 'ABM12') : ?>
                                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                        <input type='hidden' name='lrn' value='<?php echo $row_abm12['lrn']; ?>'>
