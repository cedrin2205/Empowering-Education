...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["submit_add_registrar"])) {
        ...
        $role = "standby-gas12"; // Set role to standby-gas12
        ...
        $lrn = sanitizeInput($_POST['lrn']);
        $new_role = "GAS12"; // Set role to GAS12
...
// Fetch data from the registrar table for GAS12 role and standby-gas12 role
$sql_gas12 = "SELECT r.lrn, r.role, CASE WHEN u.lrn IS NULL THEN 'Pending' ELSE 'Registered' END AS registration_status
            FROM registrar r
            LEFT JOIN users u ON r.lrn = u.lrn
            WHERE r.role IN ('GAS12', 'standby-gas12')";
$result_gas12 = mysqli_query($conn, $sql_gas12);
?>
...
<head>
    ...
    <title>Registrar Table (GAS12)</title>
...
<body>
    <div class="container mt-4">
        <h2 style="font-size: medium;">Registrar Table (GAS12)</h2>
        ...
        <table class='table table-striped mt-4'>
            ...
            <tbody>
                <?php if(mysqli_num_rows($result_gas12) > 0) : ?>
                    <?php while ($row_gas12 = mysqli_fetch_assoc($result_gas12)) : ?>
                        <tr>
                            <td><?php echo $row_gas12['lrn']; ?></td>
                            <td><?php echo $row_gas12['role']; ?></td>
                            <td><?php echo $row_gas12['registration_status']; ?></td>
                            <td>
                                <?php if ($row_gas12['role'] !== 'GAS12') : ?>
                                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                        <input type='hidden' name='lrn' value='<?php echo $row_gas12['lrn']; ?>'>
