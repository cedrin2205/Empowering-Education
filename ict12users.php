...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["submit_add_registrar"])) {
        ...
        $role = "standby-ict12"; // Set role to standby-ict12
        ...
        $lrn = sanitizeInput($_POST['lrn']);
        $new_role = "ICT12"; // Set role to ICT12
...
// Fetch data from the registrar table for ICT12 role and standby-ict12 role
$sql_ict12 = "SELECT r.lrn, r.role, CASE WHEN u.lrn IS NULL THEN 'Pending' ELSE 'Registered' END AS registration_status
            FROM registrar r
            LEFT JOIN users u ON r.lrn = u.lrn
            WHERE r.role IN ('ICT12', 'standby-ict12')";
$result_ict12 = mysqli_query($conn, $sql_ict12);
?>
...
<head>
    ...
    <title>Registrar Table (ICT12)</title>
...
<body>
    <div class="container mt-4">
        <h2 style="font-size: medium;">Registrar Table (ICT12)</h2>
        ...
        <table class='table table-striped mt-4'>
            ...
            <tbody>
                <?php if(mysqli_num_rows($result_ict12) > 0) : ?>
                    <?php while ($row_ict12 = mysqli_fetch_assoc($result_ict12)) : ?>
                        <tr>
                            <td><?php echo $row_ict12['lrn']; ?></td>
                            <td><?php echo $row_ict12['role']; ?></td>
                            <td><?php echo $row_ict12['registration_status']; ?></td>
                            <td>
                                <?php if ($row_ict12['role'] !== 'ICT12') : ?>
                                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                        <input type='hidden' name='lrn' value='<?php echo $row_ict12['lrn']; ?>'>
