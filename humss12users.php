...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["submit_add_registrar"])) {
        ...
        $role = "standby-humss12"; // Set role to standby-humss12
        ...
        $lrn = sanitizeInput($_POST['lrn']);
        $new_role = "HUMMS12"; // Set role to HUMMS12
...
// Fetch data from the registrar table for HUMMS12 role and standby-humss12 role
$sql_humms12 = "SELECT r.lrn, r.role, CASE WHEN u.lrn IS NULL THEN 'Pending' ELSE 'Registered' END AS registration_status
            FROM registrar r
            LEFT JOIN users u ON r.lrn = u.lrn
            WHERE r.role IN ('HUMMS12', 'standby-humss12')";
$result_humms12 = mysqli_query($conn, $sql_humms12);
?>
...
<head>
    ...
    <title>Registrar Table (HUMMS12)</title>
...
<body>
    <div class="container mt-4">
        <h2 style="font-size: medium;">Registrar Table (HUMMS12)</h2>
        ...
        <table class='table table-striped mt-4'>
            ...
            <tbody>
                <?php if(mysqli_num_rows($result_humms12) > 0) : ?>
                    <?php while ($row_humms12 = mysqli_fetch_assoc($result_humms12)) : ?>
                        <tr>
                            <td><?php echo $row_humms12['lrn']; ?></td>
                            <td><?php echo $row_humms12['role']; ?></td>
                            <td><?php echo $row_humms12['registration_status']; ?></td>
                            <td>
                                <?php if ($row_humms12['role'] !== 'HUMMS12') : ?>
                                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                        <input type='hidden' name='lrn' value='<?php echo $row_humms12['lrn']; ?>'>
