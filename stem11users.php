...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["submit_add_registrar"])) {
        ...
        $role = "standby-stem11"; // Set role to standby-stem11
        ...
        $lrn = sanitizeInput($_POST['lrn']);
        $new_role = "STEM"; // Set role to STEM
...
// Fetch data from the registrar table for STEM role and standby-stem11 role
$sql_stem = "SELECT r.lrn, r.role, CASE WHEN u.lrn IS NULL THEN 'Pending' ELSE 'Registered' END AS registration_status
            FROM registrar r
            LEFT JOIN users u ON r.lrn = u.lrn
            WHERE r.role IN ('STEM', 'standby-stem11')";
$result_stem = mysqli_query($conn, $sql_stem);
?>
...
<head>
    ...
    <title>Registrar Table (STEM)</title>
...
<body>
    <div class="container mt-4">
        <h2 style="font-size: medium;">Registrar Table (STEM)</h2>
        ...
        <table class='table table-striped mt-4'>
            ...
            <tbody>
                <?php if(mysqli_num_rows($result_stem) > 0) : ?>
                    <?php while ($row_stem = mysqli_fetch_assoc($result_stem)) : ?>
                        <tr>
                            <td><?php echo $row_stem['lrn']; ?></td>
                            <td><?php echo $row_stem['role']; ?></td>
                            <td><?php echo $row_stem['registration_status']; ?></td>
                            <td>
                                <?php if ($row_stem['role'] !== 'STEM') : ?>
                                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                        <input type='hidden' name='lrn' value='<?php echo $row_stem['lrn']; ?>'>
