...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["submit_add_registrar"])) {
        ...
        $role = "standby-gas11"; // Set role to standby-gas11
...
// Fetch data from the registrar table for GAS role and standby-gas11 role
$sql_gas = "SELECT r.lrn, r.role, CASE WHEN u.lrn IS NULL THEN 'Pending' ELSE 'Registered' END AS registration_status
            FROM registrar r
            LEFT JOIN users u ON r.lrn = u.lrn
            WHERE r.role IN ('GAS', 'standby-gas11')";
$result_gas = mysqli_query($conn, $sql_gas);
?>
...
<head>
    ...
    <title>Registrar Table (GAS)</title>
...
<body>
    <div class="container mt-4">
        <h2 style="font-size: medium;">Registrar Table (GAS)</h2>
        ...
        <table class='table table-striped mt-4'>
            ...
            <tbody>
                <?php if(mysqli_num_rows($result_gas) > 0) : ?>
                    <?php while ($row_gas = mysqli_fetch_assoc($result_gas)) : ?>
                        <tr>
                            <td><?php echo $row_gas['lrn']; ?></td>
                            <td><?php echo $row_gas['role']; ?></td>
                            <td><?php echo $row_gas['registration_status']; ?></td>
                            <td>
                                <?php if ($row_gas['role'] !== 'GAS') : ?>
                                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                        <input type='hidden' name='lrn' value='<?php echo $row_gas['lrn']; ?>'>
