...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["submit_add_registrar"])) {
        ...
        $role = "standby-ict11"; // Set role to standby-ict11
...
// Fetch data from the registrar table for ICT role and standby-ict11 role
$sql_ict = "SELECT r.lrn, r.role, CASE WHEN u.lrn IS NULL THEN 'Pending' ELSE 'Registered' END AS registration_status
            FROM registrar r
            LEFT JOIN users u ON r.lrn = u.lrn
            WHERE r.role IN ('ICT', 'standby-ict11')";
$result_ict = mysqli_query($conn, $sql_ict);
?>
...
<head>
    ...
    <title>Registrar Table (ICT)</title>
...
<body>
    <div class="container mt-4">
        <h2 style="font-size: medium;">Registrar Table (ICT)</h2>
        ...
        <table class='table table-striped mt-4'>
            ...
            <tbody>
                <?php if(mysqli_num_rows($result_ict) > 0) : ?>
                    <?php while ($row_ict = mysqli_fetch_assoc($result_ict)) : ?>
                        <tr>
                            <td><?php echo $row_ict['lrn']; ?></td>
                            <td><?php echo $row_ict['role']; ?></td>
                            <td><?php echo $row_ict['registration_status']; ?></td>
                            <td>
                                <?php if ($row_ict['role'] !== 'ICT') : ?>
                                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                        <input type='hidden' name='lrn' value='<?php echo $row_ict['lrn']; ?>'>
